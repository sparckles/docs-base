...
def update_crime(db: Session, crime_id: int, crime: Crime):
    db_crime = get_crime(db, crime_id)
    if db_crime is None:
        return None
    for key, value in crime.dict().items():
...
async def update_crime(request, crime_id: int):
    crime = jsonify(request.body)["crime"]
