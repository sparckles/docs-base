...
async def search_crimes(crime_type: str = None, date: str = None, location: str = None, status: str = None, db: Session = Depends(get_db_session)):
...
Batman decided to visualize the crime data on a heat map to help the police identify high-crime areas in Gotham City. He integrated a mapping library and created a new endpoint to retrieve the geospatial data required to generate the heat map.

```python {{ title: 'Crime Heat Map' }}
@app.get("/crimes/heatmap")
async def get_crime_heatmap_data(db: Session = Depends(get_db_session)):
...
<div className="not-prose">
  <Button
    href="/documentation/monitoring_and_logging"
    variant="text"
    arrow="right"
